...
export function generatePrintHtml(allTopics, logoB64 = null, includeTooltips = true) {
  ...
  const sections = allTopics.map((topic, ti) => {
    ...
    const stepsHtml = topic.steps.map(step => {
      ...
      const slideTitleHtml = step.slideTitle ? `<div class="step-slide-title">${escHtml(step.slideTitle)}</div>` : '';
      // DKP element highlights scaled for print
      let dkpElementsHtml = '';
      ...
      // DKP branch indicator
      const branches = step.navigation?.branches;
      const branchHtml = (branches?.length > 0) ? `<div class="branch-row">${branches.map(b => `<span class="branch-tag">&#x2192; ${escHtml(b.label?.slice(0, 40) || b.target)}</span>`).join('')}</div>` : '';
      return `<div class="step-wrap"><div class="step-label">Step ${step.stepNum}</div>${slideTitleHtml}<div class="sc" style="width:${DISP_W}px;height:${DISP_H}px;">${imgEl}${hotspotEl}${bubbleEl}${dkpElementsHtml}</div>${captionHtml}${branchHtml}</div>`;
    }).join('');
...
<style>
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important;}
  ...
  .cover{background:linear-gradient(135deg,#3a3660,#25223B);color:#fff;padding:50px 40px;margin-bottom:30px;}.cover h1{font-size:26px;font-weight:700;margin-bottom:8px;}.cover-date{font-size:11px;opacity:.6;margin-top:6px;}
  ...
  .topic-section{padding:0 30px 20px;}.topic-header{display:flex;align-items:center;gap:12px;background:linear-gradient(90deg,#3a3660,#25223B);padding:12px 18px;border-radius:6px;margin-bottom:16px;color:#fff;}
  ...
  .step-wrap{margin-bottom:22px;break-inside:avoid;}.step-label{font-size:10px;font-weight:700;color:#FF6B18;text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px;}.step-slide-title{font-size:11px;font-weight:600;color:#374151;margin-bottom:5px;}
  ...
  .branch-row{display:flex;flex-wrap:wrap;gap:5px;margin-top:6px;}.branch-tag{font-size:10px;color:#92400e;background:#fff7ed;border:1px solid #fed7aa;border-radius:99px;padding:2px 8px;white-space:nowrap;}
  ...
  function doOverlay(){
    ...
    if(s.elements&&s.elements.length){
      ...
      for(const el of s.elements){
        ...
        // Tooltip
        const tipText=el.content?el.content.slice(0,160):'';
...
function updateUI(s){
  document.getElementById('tb-topic').textContent=(s.slideTitle||s.topic)||(s.type==='cover'?'Overview':'');
  document.getElementById('tb-fill').style.width=(SLIDES.length>1?Math.round(cur/(SLIDES.length-1)*100):100)+'%';
  const branchCount=s.branches&&s.branches.length?s.branches.length:0;
  const branchHint=branchCount>1?' ('+branchCount+' paths)':'';
  document.getElementById('tb-label').textContent=s.type==='step'?'Step '+s.stepNum+' of '+s.totalSteps+branchHint:(s.type==='cover'?'Overview':'Introduction');
  ...
  for (let ri = 0; ri < refs.length; ri++) {
    ...
    for (const el of elements) {
      for (const act of (el.actions || [])) {
        ...
        branches.push({ sourceId: el.id, label: el.content?.slice(0, 80) || el.id, event: act.event, target: act.target });
      }
    }
