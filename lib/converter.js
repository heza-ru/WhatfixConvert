...
document.addEventListener('keydown',e=>{if(e.key==='ArrowRight'||e.key===' '){e.preventDefault();go(1);}if(e.key==='ArrowLeft'){e.preventDefault();go(-1);}if(e.key==='Escape'&&window.parent!==window)window.parent.postMessage('preview:close','*');});
render(0);
</script></body></html>`;
}
  ...
  for (let ri = 0; ri < refs.length; ri++) {
    ...
    if (xmlEntry) {
      try {
        ...
        // non-fatal: fall through to JS-based element extraction
      }
    }
